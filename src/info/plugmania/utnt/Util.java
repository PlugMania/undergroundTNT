...
public class Util{
	...
	static undergroundTNT plugin;
	
	public Util(undergroundTNT instance) {
		plugin = instance;
	}
		...
		try {	
			...
			while (((str = in.readLine()) != null) && !endCheck) {				
				...
				if(m.find()) {
					...
					if(curVal<chkVal){
						String msg = ChatColor.AQUA + "[utnt] New version " + ChatColor.GOLD + "v" + m.group(0) + ChatColor.AQUA + " is out! You are running " + ChatColor.GRAY + "v" + plugin.getDescription().getVersion() + ChatColor.AQUA + ". Get the new version is at " + ChatColor.GOLD + "http://dev.bukkit.org/server-mods/" + plugin.getDescription().getName();
						...
						sender.sendMessage(ChatColor.AQUA + "[utnt] Newest version " + ChatColor.GOLD + plugin.getDescription().getVersion() + ChatColor.AQUA + " already running.");	
					}
					endCheck = true;
				}
			}
